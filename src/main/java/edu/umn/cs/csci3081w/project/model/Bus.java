...
public class Bus implements BusObserver {
  ...
  public void updateBusData() {
    ...
    if (currentRoute() == outgoingRoute) {
      busData = new MaroonColorDecorator(busData.getId(),
          busData.getPosition(), busData.getNumPassengers(), busData.getCapacity());
    } else {
      busData = new GoldColorDecorator(busData.getId(),
          busData.getPosition(), busData.getNumPassengers(), busData.getCapacity());
    }

...
public class VisualizationSimulator {
  ...
  private SaveSimulationSingleton singletonWriter;

  public void setTimeSinceLastBus(List<Integer> ints) {
    this.timeSinceLastBus = ints;
  }

  public void setBusses(List<Bus> bus) {
    this.busses = bus;
  }

  public void setPrototypeRoutes(List<Route> route) {
    this.prototypeRoutes = route;
  }
  ...
  public void update() throws IOException {
    if (!paused) {
      ...
      if (simulationTimeElapsed == 1) {
        singletonWriter = SaveSimulationSingleton.getInstance();
        //Clean the file if a new simulation has been started
        if (singletonWriter.simData.csvFile.isFile()) {
          new FileWriter(singletonWriter.simData.csvFile.getName(), false).close();
        }
      }
      singletonWriter.simData.writeSimulationData(simulationTimeElapsed, busses, prototypeRoutes);
    }
  }

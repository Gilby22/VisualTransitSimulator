...
public class MyWebServer implements WebInterface {
  ...
  public void setBussesWeb(List<BusData> busses) {
    this.busses = busses;
  }
  ...
  public void updateBus(BusData tempData, boolean deleted) {
    ...
    if (found != null) {
      ...
      busses.get(index).setRed(tempData.getRed());
      busses.get(index).setGreen(tempData.getGreen());
      busses.get(index).setBlue(tempData.getBlue());
      busses.get(index).setAlpha(tempData.getAlpha());
